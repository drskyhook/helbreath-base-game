...
```typescript
this.player = new Player(this, playerWorldX, playerWorldY, 1, this.soundManager, map, gear, movementSpeedMs);
```
...
- **Critical strike**: `CriticalStrikeProjectile` for `AttackType.Knockback` melee
...
- `AttackType.Knockback` — knockback 1 cell away from attacker, then take damage animation
...
### GroundItem / Server Ground State

- `player.requestPickUp()` when clicking own cell
- On PickUp animation end, `Player` sends the server pickup-complete request
- `GameWorld` renders top-most ground items from authoritative `ground_states_*` packets
...
| `PLAYER_PICKUP_PERFORMED_RECEIVED` | Server → GameWorld | Sync remote pickup animation |
