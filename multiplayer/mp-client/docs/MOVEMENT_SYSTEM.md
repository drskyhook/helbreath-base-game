...
| `movementSpeedMs` | Per-tile step duration in ms (align with proto; set via `GameObjectConfig`). |
...
2. `progress = min(movementElapsedTime / movementSpeedMs, 1)`.
...
- **Run:** Full speed, run animation; tile step uses `movementSpeedMs` from server/dialog.
- **Walk:** Walk animation at the same per-tile duration as run in the current client implementation (visual differs; duration source is still `movementSpeedMs`).
...
- Base per-tile duration comes from server monster config (`movementSpeedMs` on spawn); packets can update it (e.g. Chill). `0` means immobile.
...
- **Trigger:** `AttackType.Knockback` on damage.
