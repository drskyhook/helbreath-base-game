...
export function NativeOverlayCanvas({ gameRef }: NativeOverlayCanvasProps) {
    ...
    useLayoutEffect(() => {
        ...
        const syncBounds = (allowRetry = true) => {
            ...
            if (loginButton) {
                const buttonWidth = 140 * scaleX;
                const buttonHeight = 40 * scaleY;
                setStylePixelValue(loginButton, 'left', overlayLeft + contentWidth / 2 - buttonWidth / 2);
                setStylePixelValue(loginButton, 'top', overlayTop + contentHeight * 0.65 - buttonHeight / 2);
                setStylePixelValue(loginButton, 'width', buttonWidth);
                setStylePixelValue(loginButton, 'height', buttonHeight);
                loginButton.style.fontSize = `${Math.max(20, Math.round(20 * Math.max(scaleX, scaleY)))}px`;
            }
        ...
        const hideHealthBar = () => {
            ...
            if (floatingTextsRef.current.length === 0 && !mapLoadingVisibleRef.current) {
                clearOverlay(context, boundsRef.current);
            }
        };
        ...
        const hideMapLoading = () => {
            ...
            mapLoadingDrawLoggedRef.current = false;
            if (floatingTextsRef.current.length === 0 && !healthBarRef.current) {
                clearOverlay(context, boundsRef.current);
            }
        };
            ...
            {loginButtonVisible ? (
                <>
                    ...
                    <button
                        ref={loginButtonRef}
                        type="button"
                        className="native-overlay-login-button"
                        onClick={() => EventBus.emit(NATIVE_OVERLAY_LOGIN_REQUESTED)}
                    >
                        Log in
                    </button>
                </>
